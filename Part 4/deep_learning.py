...
# SPECIFICATION: A simple program that iterates through different parameters for a deep neural network and test for the highest performance (accuracy) model and prints some information about it
...
for h in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for n in n_neurons:                     #looking or the best parameters w.r.t the number of neurons
        for l in l_rate:                    #looking or the best parameters w.r.t the learning rate
            ...
            _, tempAccuracy = model.evaluate(X_test, y_test, verbose=0)
...
print("Highest accuracy final: " + str(highestAccuracy))
